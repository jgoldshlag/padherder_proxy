...
def do_sync(raw_captured_data):
	session = requests.Session()
	settings_file = open('padherder_proxy_settings.txt', 'r')
	settings = settings_file.readlines()
	settings_file.close()
	session.auth = (settings[0], settings[1])
		...
		if mon_array[0] in existing_monsters:
			...
			if mon_array[1] > existing_data['current_xp'] or \
			   ...
			   mon_array[9] > existing_data['current_awakening']:
				...
				if r.status_code == requests.codes.ok:
					print 'Updated monster #%d: %s' % (existing_data['id'], ', '.join(k for k in update_data.keys() if k != 'monster'))
				else:
					print 'Failed updating monster #%d: %s %s' % (existing_data['id'], r.status_code, r.content)
			...
			if found_id is not None:
				...
				if r.status_code == requests.codes.ok:
					print 'Updated monster #%d: %s' % (jp_id, ', '.join(k for k in update_data.keys() if k != 'monster'))
				else:
					print 'Failed updating monster #%d: %s %s' % (jp_id, r.status_code, r.content)
			else:
				r = session.post(URL_MONSTER_CREATE, update_data)
				if r.status_code == requests.codes.ok or r.status_code == 201:
					print 'Created monster %r: %s' % (monster_data[jp_id]['name'], ', '.join(k for k in update_data.keys() if k != 'monster'))
				else:
					print 'Failed creating monster %r: %s %s' %(monster_data[jp_id]['name'], r.status_code, r.content)
	
	for mon in existing_monsters.values():
		r = session.delete(mon['url'])

		if r.status_code == 204:
			print 'Removed monster %r' % mon['id']
		else:
			print 'Failed to remove monster %r' % mon['id']
...
def serve_app():
	config = proxy.ProxyConfig(port=8080, host=socket.gethostbyname(socket.gethostname()))
	server = ProxyServer(config)
	master = dump.DumpMaster(server, dump.Options(app_host='mitm.it', app_port=80, app=True))
	master.run()

def main():
	d = {}
	thread.start_new_thread(dnsproxy.serveDNS, (d,))
	thread.start_new_thread(serve_app, ())
	config = proxy.ProxyConfig(port=443, host=socket.gethostbyname(socket.gethostname()), mode='reverse', upstream_server=cmdline.parse_server_spec('https://api-na-iosv2.padsv.gungho.jp:443/'))
	server = ProxyServer(config)
	m = PadMaster(server)
	m.run()
